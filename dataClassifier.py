...
def enhancedPacmanFeatures(state, action):
    ...
    features['nearest_ghost'] = min(
        [(util.manhattanDistance(ghost.getPosition(), pacmanPosition))for ghost in ghostStates]
    )
    features['ghost_count'] = sum(
        [(util.manhattanDistance(ghost.getPosition(), pacmanPosition)<3 )for ghost in ghostStates]
    )

    if len(foods) > 0:
        features['nearest_ghost'] = min(
        [(util.manhattanDistance(foods , pacmanPosition))for food in foods]
        )
    
    if len(capsules) > 0:
        features['capsules'] = min(
        [util.manhattanDistance(capsule , pacmanPosition) for capsule in capsules]) < min([util.manhattanDistance(capsule , pacmanPosition) 
        for capsule in currentCapsules])

    features['scare_times'] = sum([(ghost.scared_Timer == 0 ) for ghost in ghostStates])
